...
dt3$Zjc02 <- NULL
dt3$Zkauditdic <- NULL
dt3$Zloutromotsono <- NULL
dt3$Zloutromotsono2 <- NULL
...
dt3$Zlauditpreocup <- as.numeric(dt3$Zlauditpreocup)

dt3$Zlcomp1sono <- as.numeric(dt3$Zlcomp1sono)
dt3$Zlcomp2sono  <- as.numeric(dt3$Zlcomp2sono)
dt3$Zlduracao <- as.numeric(dt3$Zlduracao)
dt3$Zlcomp3sono <- as.numeric(dt3$Zlcomp3sono)
dt3$Zlcomp4sono <- as.numeric(dt3$Zlcomp4sono)
dt3$Zlcomp5sono <- as.numeric(dt3$Zlcomp5sono)
dt3$Zlcomp6sono <- as.numeric(dt3$Zlcomp6sono)
dt3$Zlpsqi  <- as.numeric(dt3$Zlpsqi )
dt3$Zlcomp6sono <- as.numeric(dt3$Zlcomp6sono)
map_chr(dt3, class)
### HOLD OUT ####
...
treino_onda4 <- train_matrix[,c("bipolar1","ksentfel","kpesfel", "kcompfel", "kvidafel","kvidatri","ktabagismodiario", "kprosau", "kmorpar", "kmorprox","kprobdin","kmudcas", "kternam", "krixabai","kinsbai", "kbriga", "kapanhou","kassalto","kauditdic", "kmotpsiq3", "Zkqvfisico", "Zkqvpsico", "Zkqvsocial", "Zkqvambi","ZkSRQdepre_anx", "ZkSRQsomatic", "ZkSQRenergia", "ZkSRQsentidepre", "histpsiquiatri", "Zkescadol", "Zkibem", "Zkfeliz", "cocaina_lifetime", "maconha_lifetime", "alucino_lifetime", "solvente_lifetime", "alcool_lifetime")]
...
results <- rfe(treino_onda4[,c(2:37)], treino_onda4$bipolar1, rfeControl=control, method="rf", metric = "ROC", trControl=trainctrl)
...
rose_outside_NET <- train(bipolar1 ~., data = rose_train, method = "glmnet", trControl = ctrl.rf_rose_NET, tuneLength = 10, metric = "ROC")


library(e1071)
test_pred_up_NET <- predict(up_outside_NET, newdata = data_testing[,-1])
matrixConfusao_up_NET <- confusionMatrix(test_pred_up_NET, data_testing$bipolar1, positive = "yes")
...
confusionMatrix(p_up_NET, data_testing$bipolar1)
print(confusionMatrix(p_up_NET, data_testing$bipolar1, positive = "yes"))
r_up_NET <- roc (data_testing$bipolar1, p_prob_up_NET[,"yes"])
plot(r_up_NET)
r_up_NET$auc
# matrix de confusao 
library(e1071)
